...
class UserTest(unittest.TestCase):
    ...
    def test_create_address_success(self):
        response = self.client.post(ADDRESSES_URL, json = {"address_line_1": "333", "city": "Hyderabad", "state": "TG", "pin": 500001}, headers = {"Authorization": f"Bearer {self.access_token}"})
        print(f"API Response: {response.json}")
        self.assertEqual(response.status_code, 201)
        self.assertEqual(len(response.json), 2)

    def test_create_address_invalid_state_error(self):
        response = self.client.post(ADDRESSES_URL, json = {"address_line_1": "333", "city": "Hyderabad", "state": "AB", "pin": 500001}, headers = {"Authorization": f"Bearer {self.access_token}"})
